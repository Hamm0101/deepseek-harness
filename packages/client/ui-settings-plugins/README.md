...
- **The empty state counts visible cards, not registered ones** — a card whose namespace this deployment does not expose renders nothing and does not count toward the tab's total, so a deployment that exposes none shows the empty line rather than a blank list. The count is a live projection of the cards' availability, so a namespace coming or going under the deployment moves the tab between the empty line and the card list.
