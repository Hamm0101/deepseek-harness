...
- **Remote browsers get durable settings only when the deployment trusts them** — a scope bound in a non-loopback browser starts `unavailable` and never crosses the wire until the connection handshake reports `configurationTrusted` (the browser's authority is a declared `trustedHosts` entry), which upgrades the scope to host transport; rows it backs stay inert for untrusted remote browsers.
